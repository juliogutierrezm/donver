...
function toPartySummary(item, userId) {
  ...
  const phone = typeof item?.["phone"] === "string" && item["phone"].trim() ? item["phone"] : void 0;
  const avatarUrl = typeof item?.["avatar_url"] === "string" && item["avatar_url"].trim() ? item["avatar_url"] : typeof item?.["avatarUrl"] === "string" && item["avatarUrl"].trim() ? item["avatarUrl"] : void 0;
  return {
    id: userId,
...
function toPetSummary(item) {
  ...
  return {
    ...
    age: typeof item["age"] === "number" ? item["age"] : void 0,
    description: typeof item["description"] === "string" && item["description"].trim() ? item["description"] : void 0,
    photos: Array.isArray(item["photos"]) ? item["photos"].map(String) : [],
    medical_notes: typeof item["medical_notes"] === "string" && item["medical_notes"].trim() ? item["medical_notes"] : void 0
  };
}
