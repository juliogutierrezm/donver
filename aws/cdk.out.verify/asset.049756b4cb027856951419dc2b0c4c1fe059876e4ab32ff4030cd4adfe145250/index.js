...
// lambda/handlers/auth-bootstrap/index.ts
var index_exports = {};
...
function deriveRolesFromGroups(groups) {
...
async function handler(event) {
  try {
    ...
    if (!existing.Item) {
      ...
      await ddb.send(new import_lib_dynamodb4.PutCommand({ TableName: CORE_TABLE_NAME, Item: profile }));
      return ok(await normalizeProfileWithStatus(profile, derivedRoles.length ? derivedRoles : ["owner"]));
    }
    const item = existing.Item;
    const signupIntent = item.signup_intent === "caregiver" ? "caregiver" : null;
    const roles = normalizeRoles(item.roles ?? item.role ?? derivedRoles);
    const nextRoles = roles.length ? roles : signupIntent === "caregiver" ? [] : derivedRoles.length ? derivedRoles : ["owner"];
    ...
    const updated = await ddb.send(new import_lib_dynamodb4.UpdateCommand({
      TableName: CORE_TABLE_NAME,
      Key: coreKeys.userProfile(sub),
    ...
    return ok(await normalizeProfileWithStatus(updated.Attributes ?? {}, nextRoles));
  } catch (err) {
