...
// lambda/handlers/list-owner-bookings/index.ts
var index_exports = {};
...
function toSpaceSummary(item) {
  ...
  const caregiverId = typeof item?.["caregiver_id"] === "string" && item["caregiver_id"].trim() ? item["caregiver_id"] : "";
  return {
  ...
  try {
    ...
    const enriched = await Promise.all(
      bookings.map(async (booking) => {
        ...
        const space = spaceId ? await safeGetItem(coreKeys.space(spaceId)) : void 0;
        const spaceSummary = toSpaceSummary(space);
        const caregiver = spaceSummary.caregiverId ? await safeGetItem(coreKeys.userProfile(spaceSummary.caregiverId)) : void 0;
